...
- `book.php` / `contact.php` — forms save the enquiry to `data/bookings.json` (admin Bookings tab) and forward it to WhatsApp
- `blog.php`, `blog-post.php` — blog (index in `data/posts.json`, bodies in `data/posts/*.html`); served via clean URLs `/blog/` and `/blog/<slug>`
- `admin/index.php` — the admin panel (see below)
- `.htaccess` — clean URL rewrites + 301 redirects from old `route.php?slug=...-taxi`, `blog.php` and `blog-post.php?slug=...` links
...
- **Bookings** — inbox of every enquiry from the booking form, contact form and fare estimator (name, mobile with tap-to-call/WhatsApp, route, date, message). Mark as New/Contacted/Done or delete. Saved to `data/bookings.json`.
